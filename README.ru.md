...
На реальном iPhone нестабильность подключения была устранена сочетанием двух
клиентских настроек Shadowrocket: `Up Mb/s = 100`, `Down Mb/s = 100` и
`Keepalive = 10`. Это подтверждённый практический результат одного подключения,
а не универсальные значения для любого оператора:
...
`100/100` следует считать проверенным значением именно для описанного теста. В
более медленной или перегруженной сети его нужно уменьшать до устойчивой
