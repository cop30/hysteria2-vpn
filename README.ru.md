...
документации. Для iOS в мобильной сети разумная начальная настройка Keepalive —
10 секунд. IPv6 включайте только после проверки IPv6-выхода VPS.

### Shadowrocket на iPhone: проверенный профиль стабильности

На реальном iPhone сочетание `Up Mb/s = 100`, `Down Mb/s = 100` и
`Keepalive = 10` заметно улучшило стабильность Shadowrocket, но не устранило
обрывы полностью. Похожее поведение наблюдалось и на Windows, поэтому это
временное смягчение проблемы, а не доказанное окончательное исправление и не
универсальные значения для любого оператора:

1. Откройте информацию/редактирование узла Hysteria 2.
2. Установите `Keepalive` в `10` секунд.
3. Для воспроизведения проверенного профиля установите `Up Mb/s = 100` и
   `Down Mb/s = 100`.
4. Сохраните профиль, выключите и снова включите VPN.

Почему это может помочь: Brutal использует указанную скорость как целевую и при
потерях пытается компенсировать их дополнительной отправкой. Официальная
документация Hysteria предупреждает, что значение выше реальной пропускной
способности сети вызывает перегрузку, лишний трафик и нестабильность. Лимит ниже
реальной скорости допустим и фактически работает как ограничитель. Поэтому
`100/100` следует считать только значением, давшим улучшение в описанном тесте.
В более медленной или перегруженной сети его нужно уменьшать до устойчивой
скорости, а не копировать вслепую.

Если ручные значения ухудшают связь, верните поля bandwidth в пустое/авто:
клиент тогда должен использовать non-Brutal congestion control (по умолчанию
BBR), если Shadowrocket реализует это согласно Hysteria core.

Маршрутизация и IPv6 не были частью подтверждённого исправления. IPv6 всё равно
не следует включать до проверки IPv6-выхода VPS. Также обновите Shadowrocket
через App Store. Не меняйте только в клиенте порт
`443/udp`: порт должен одновременно совпадать на сервере, в URI, UFW и firewall
провайдера. Не отключайте Salamander/Obfuscation для профиля этого проекта —
сервер ожидает тот же секрет, и одностороннее отключение разорвёт подключение.
...
Этот репозиторий **не устанавливает и не обновляет** Shadowrocket, Hiddify или
другое клиентское приложение. Их обновляют через App Store, Google Play,
официальный магазин/репозиторий конкретного приложения. Обновление сервера не
означает автоматическую замену клиента, и для обновления приложения не нужно
создавать новый пароль или QR.
